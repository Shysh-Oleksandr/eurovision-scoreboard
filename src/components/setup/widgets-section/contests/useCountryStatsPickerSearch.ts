import { useEffect, useMemo, useRef, useState } from 'react';

import { CATEGORY_ORDER } from '../../constants';

import { useDebounce } from '@/hooks/useDebounce';
import { BaseCountry } from '@/models';

/**
 * Groups countries/custom entries by category (continents + Custom) for the entry stats picker.
 * Simplified from useVotersCountriesSearch (no stage-based groups).
 */
export const useCountryStatsPickerSearch = (countries: BaseCountry[]) => {
  const [countriesSearch, setCountriesSearch] = useState('');
  const [expandedCategories, setExpandedCategories] = useState<
    Record<string, boolean>
  >({});
  const debouncedSearch = useDebounce(countriesSearch, 300);
  const prevDebouncedSearchRef = useRef<string | undefined>(undefined);

  const handleCountriesSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    setCountriesSearch(e.target.value);
  };

  const clearSearch = () => {
    setCountriesSearch('');
  };

  const { groupedCountries, sortedCategories } = useMemo(() => {
    const groups: Record<string, BaseCountry[]> = {};
    const q = debouncedSearch.trim().toLowerCase();

    const filtered = countries.filter((country) =>
      country.name.toLowerCase().includes(q),
    );

    filtered.forEach((country) => {
      if (country.isImported) {
        if (!groups['Imported']) groups['Imported'] = [];
        groups['Imported'].push(country);

        return;
      }

      const category = country.category || 'Other';

      if (!groups[category]) groups[category] = [];
      groups[category].push(country);
    });

    if (!groups['Custom']) groups['Custom'] = [];
    if (!groups['Imported']) groups['Imported'] = [];

    Object.keys(groups).forEach((category) => {
      groups[category].sort((a, b) => a.name.localeCompare(b.name));
    });

    const sortedCategories = Object.keys(groups).sort((a, b) => {
      const aIndex = CATEGORY_ORDER.indexOf(a);
      const bIndex = CATEGORY_ORDER.indexOf(b);

      if (aIndex === -1 && bIndex === -1) return a.localeCompare(b);
      if (aIndex === -1) return 1;
      if (bIndex === -1) return -1;

      return aIndex - bIndex;
    });

    return { groupedCountries: groups, sortedCategories };
  }, [countries, debouncedSearch]);

  useEffect(() => {
    if (debouncedSearch !== prevDebouncedSearchRef.current) {
      if (debouncedSearch.trim()) {
        const next: Record<string, boolean> = {};

        sortedCategories.forEach((category) => {
          next[category] = true;
        });
        setExpandedCategories(next);
      } else {
        setExpandedCategories({});
      }
    }
    prevDebouncedSearchRef.current = debouncedSearch;
  }, [debouncedSearch, sortedCategories]);

  const handleToggleCategory = (category: string) => {
    setExpandedCategories((prev) => ({
      ...prev,
      [category]: !prev[category],
    }));
  };

  return {
    countriesSearch,
    handleCountriesSearch,
    clearSearch,
    expandedCategories,
    handleToggleCategory,
    groupedCountries,
    sortedCategories,
  };
};
