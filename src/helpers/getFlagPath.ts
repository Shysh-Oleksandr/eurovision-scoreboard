...
const minimalSupportedRoundFlags = ['FR', 'IE', 'IT', 'TR'];
...
const minimalSupportedSquareFlags = [
  'AU',
  'BY',
  'FR',
  'GE',
...
export const getFlagPath = (
  ...
  enableMinimalisticFlags: boolean = false,
): string => {
  if (typeof country === 'string') {
    if (
      (shape === 'round' || shape === 'round-border') &&
      (enableMinimalisticFlags
        ? supportedRoundFlags.includes(country.toUpperCase())
        : minimalSupportedRoundFlags.includes(country.toUpperCase()))
    ) {
      return `/flags-shapes/round/${country.toLowerCase()}.svg`;
    } else if (
      shape === 'square' &&
      (enableMinimalisticFlags
        ? supportedSquareFlags.includes(country.toUpperCase())
        : minimalSupportedSquareFlags.includes(country.toUpperCase()))
    ) {
      return `/flags-shapes/square/${country.toLowerCase()}.svg`;
    }
  ...
  if (
    ...
    (enableMinimalisticFlags
      ...
      : minimalSupportedRoundFlags.includes(country.code.toUpperCase()))
  ) {
    return `/flags-shapes/round/${country.code.toLowerCase()}.svg`;
  } else if (
    shape === 'square' &&
    (enableMinimalisticFlags
      ? supportedSquareFlags.includes(country.code.toUpperCase())
      : minimalSupportedSquareFlags.includes(country.code.toUpperCase()))
  ) {
    return `/flags-shapes/square/${country.code.toLowerCase()}.svg`;
  }
...
export const getFlagPathForImageGeneration = (
  ...
  const flagPath = getFlagPath(country, shape, enableMinimalisticFlags);
